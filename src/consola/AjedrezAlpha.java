...
public class AjedrezAlpha {
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            int opcion = Excepciones.leerNumero("Elige una opción", 0, 6);
            switch (opcion) {
                case 0:
                ...
                case 4:
                    ...
                    break;
                case 5:
                    mostrarPNG(consulta, consola);
                    break;
                case 6:
                    mostrarJugadores(consulta);
                    break;
            }
        }
    }
    ...
    private static void iniciarNuevaPartida(AjedrezConsulta consulta, Consola consola) {
        ...
        consulta.guardarPartida(idPartida, tablero.getFEN(), tablero.getEsTurnoBlanco(), tablero.getTiempoBlancas(), tablero.getTiempoNegras(), tablero.getUltimoMovimiento(), tablero.getContadorMovimientos(), tablero.getCincuentaMovimientos());
    ...
    private static void recrearPartida(AjedrezConsulta consulta, Consola consola) {
        ...
        if (historial != null) {
            recrear(tablero, historial, consulta.obtenerInformacion(idPartida), consola);
        }
    }

    private static void mostrarPNG(AjedrezConsulta consulta, Consola consola) {
        int idPartida = 0;
        ...
        consola.mensaje(consulta.generarPGN(idPartida));
    }

    private static void mostrarJugadores(AjedrezConsulta consulta) {
        consulta.mostrarJugadores();
        int jugador = Excepciones.leerNumero("Elige un jugador", 0, 10);
        consulta.mostrarEstadisticasJugador(jugador);
    }
    ...
    private static void jugarPartida(AjedrezConsulta consulta, Consola consola, Tablero tablero, int partida) {
        ...
        while (true) {
            ...
            if (negras.tiempoAgotado()) {
                ...
                consulta.actualizarVictorias(partida, true);
                consulta.actualizarPerdidas(partida, false);
                break;
            }
            if (blancas.tiempoAgotado()) {
                ...
                consulta.actualizarVictorias(partida, false);
                consulta.actualizarPerdidas(partida, true);
                break;
            }
    ...
    private static boolean verificarFin(AjedrezConsulta consulta, Consola consola, Tablero tablero, int partida) {
        if (tablero.tablasAhogado(tablero.getEsTurnoBlanco())) {
            ...
            consulta.actualizarTablas(partida, tablero.getEsTurnoBlanco());
            consulta.actualizarPerdidas(partida, !tablero.getEsTurnoBlanco());
            return true;
        }
        ...
        if (tablero.estaEnJaque(tablero.getEsTurnoBlanco())) {
            ...
            if (tablero.estaJaqueMate(tablero.getEsTurnoBlanco())) {
                ...
                consulta.actualizarVictorias(partida, !tablero.getEsTurnoBlanco());
                consulta.actualizarPerdidas(partida, tablero.getEsTurnoBlanco());
    ...
    private static boolean comandosDelMenu(AjedrezConsulta consulta, Consola consola, String jugador, Tablero tablero, int partida, Temporizador blancas, Temporizador negras) {
        ...
        switch (opcion) {
            case 1:
                ...
                consulta.guardarPartida(partida, tablero.getFEN(), tablero.getEsTurnoBlanco(), tablero.getTiempoBlancas(), tablero.getTiempoNegras(), tablero.getUltimoMovimiento(), tablero.getContadorMovimientos(), tablero.getCincuentaMovimientos());
    ...
    private static void recrear(Tablero tablero, List<Informe> historial, String[] informacion, Consola consola) {
        consola.setTablero(tablero);
        consola.mostrarInformacionPartida(informacion[0], informacion[1], informacion[2], informacion[3]);
        int index = 0;

        while (index < historial.size()) {
            Informe informe = historial.get(index);
            tablero.restaurarPartida(informe.getFEN());
            ...
            consola.mensaje(informe.descripcion(informacion[0], informacion[1]));
            consola.mostrarTablero();
